...
class SSDALossModule(torch.nn.Module):
    def __init__(self, config, gpu_devices):
        super(SSDALossModule, self).__init__()       #####what is SSDALossModule?? Nothing on the internet....
        self.config = config
    ...
    ## broadcast values to the gpu devices
    ...
    def forward(self, indices, outputs, domain, gpu_idx):
        ...
        for i, ls in enumerate(self.loss):   ## loss list

            if (self.epoch <= self.config.loss_params.start[i]
                or self.epoch >= self.config.loss_params.end[i]
                or ls.split("-")[0] in ["cls", "tgt", "semi", "norm"]
            ):

                l = torch.zeros(1).cuda()
                ## holds for "cls-so" , "semi-condentmax","semi-entmin","tgt-condentmax","tgt-entmin"

            else:                                                      ## for "proto-each","I2C-cross",
