...
def create_image_label(image_list):
    image_index = [x.split(" ")[0] for x in open(image_list)]           ##example : dslr/images/back_pack/frame_0001.jpg
    label_list = np.array([int(x.split(" ")[1].strip()) for x in open(image_list)])
    return image_index, label_list

####so this returns a ditcionary with {0:"back_pack",1:"bike",......} according to image_list file (dslr.txt for example)
def get_class_map(image_list):          ##this gets dslr.txt as parameter
    class_map = {}
    for x in open(image_list):
        key = int(x.split(" ")[1].strip())    ## index of class
        if key not in class_map:
            class_map[key] = x.split(" ")[0].split("/")[-2]     ##name of class
    class_map = collections.OrderedDict(sorted(class_map.items()))
    return class_map

#returns total number of classes in the image_list file (dslr.txt for example)
...
###returns the postions/index of the fewshots in the whole(complete) dataset and their labels as list
...
class Imagelists(torch.utils.data.Dataset):
    def __init__(
        ...
        image_list,  ##dslr.txt
        root,             #"./data/office"
        transform=None,    #set of operations
        ...
        self.imgs = imgs        ##list of jpg file paths
        self.labels = labels    ## list of corressponding labels
        self.transform = transform
        self.target_transform = target_transform
        self.root = root
        self.ret_index = ret_index
        self.keep_in_mem = keep_in_mem
        self.loader = pil_loader      ##RGB converter of image
        ...
        if self.keep_in_mem:
            ...
            for index in range(len(self.imgs)):
                ...
                img = self.loader(path)                      ##RGB image
                if self.transform is not None:
                    img = self.transform(img)
                images.append(img)
            self.images = images        ##new set of images with opertion applied and kept in memory
...
##this returns a set of operations on the image(performed in sequence) that can be applied in the future
def get_augmentation(trans_type="aug_0", image_size=224, stat="imagenet"):
    stat = "imagenet"
    mean, std = means[stat], stds[stat]
    image_s = image_size + 32        ##256
