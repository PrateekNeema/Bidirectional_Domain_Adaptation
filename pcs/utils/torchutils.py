...
###does some necessary changes in the resnet50 model so that it can be used here
def weights_init(model):
    for layer in model.modules():
        if isinstance(layer, torch.nn.Conv2d):
            ...
            torch.nn.init.constant_(layer.bias, val=0.0)
        elif isinstance(layer, torch.nn.Linear):       ##true in this case
            torch.nn.init.xavier_normal_(layer.weight)
            if layer.bias is not None:
                torch.nn.init.constant_(layer.bias, val=0.0)   #making bias 0 (I guess)

#rteurns named and unnamed parameters in the model
...
###############
def pseudo_label_loss(x, thres=0.95, aux=True, y=None, mask=None, num_class=10):   ## x is output from images through models

    ## x is a tensor of n vectors each of length 31 for the 31 classes.
    ## Each vector is the ouput of an image through the model
    ...
    num_thres = mask.sum().item()          ## num of high confidence predictions

    # calculate loss
    out_thres, pred_thres = x[mask], pred[mask]    ## only vectors which are high confidence outputs and their predictions
    if num_thres == 0:
        loss = torch.tensor(0)
    else:
        loss = F.cross_entropy(out_thres, pred_thres)       ## computing loss
        ## cross entropy loss has arguments --> input = many vectors each of size 31, output = integer in range [0,30]

    if aux:
        num_select_per_class = [0] * num_class
        num_correct_per_class = [0] * num_class
        for i in range(num_class):
            num_select_per_class[i] += (pred_thres == i).sum().item()    ## number of high confidence predcitions per class
            if y is not None:
                num_correct_per_class[i] += (
                    ((pred_thres == i) & (pred_thres.eq(y[mask]))).sum().item()
                )           ## if actual labels are known then number of correct high confidence predictions per class

        if y is not None:
            num_correct = pred_thres.eq(y[mask]).sum().item()   ##if actual labels are known then total number of correct high confidence predictions
        else:
            num_correct = -1
